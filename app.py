        ...
        for day in week:
            if day == 0: html += "<td style='background-color:transparent;'></td>"
                ...
                if day in df_dia:
                    ...
                    if peso < 0.5: r, g, b = int(187 + (68 * (peso * 2))), 247, int(208 - (100 * (peso * 2)))
                    else: r, g, b = 255, int(247 - (45 * ((peso - 0.5) * 2))), int(108 + (94 * ((peso - 0.5) * 2)))
                    ...
                    html += f"<td style='background-color:#f8fafc; padding:8px 2px; border-radius:5px; border: 1px dashed #cbd5e1;'><div style='font-size:14px; color:#94a3b8;'>{day}</div><div style='font-size:11px; color:#cbd5e1;'>-</div></td>"
        html += "</tr>"
    html += "</table>"
    return html
...
if df_voos.empty: dt.warning("⚠️ Planilha vazia ou erro de conexão.")
    ...
    dt.sidebar.markdown("---")
    dt.sidebar.subheader("🚀 Funcionalidades")
