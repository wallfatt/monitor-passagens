...
def render_cal(df_mes, col_v, tit, taxa, is_pts, g_min, g_max):
    ...
    df_dia = df_mes.groupby(df_mes['Data partida_dt'].dt.day)[col_v].min().to_dict()
    
    for semana in cal.monthdayscalendar(ano, mes):
        cols = dt.columns(7)
        for i, day in enumerate(semana):
            if day != 0 and day in df_dia:
                val = df_dia[day]
                ...
                cols[i].markdown(f"""<div style='background-color:rgb({r},{g},{b}); padding:5px; border-radius:5px; text-align:center; font-size:11px;'>
                                 <b>{day}</b><br>{val/1000:.1f}k if is_pts else f"R${val:.0f}"</div>""", unsafe_allow_html=True)
            elif day != 0: cols[i].markdown(f"<div style='text-align:center; font-size:11px; color:#ccc;'>{day}</div>", unsafe_allow_html=True)
...
if not df.empty:
    ...
    for d in [df]:
        ...
        d['Valor'] = d['Preco clube'] if modo=="Pontos" else (d['Custo'] if modo=="Reais (Clube)" else ((d['Preco normal']/1000)*milheiro)+taxa)

    df_i = df[(df['Origem']==orig) & (df['Destino']==dest)]
    df_v = df[(df['Origem']==dest) & (df['Destino']==orig)]
    
    # Render
    meses = sorted(pd.concat([df_i['Data partida_dt'], df_v['Data partida_dt']]).dt.to_period('M').unique())
    g_min, g_max = pd.concat([df_i['Valor'], df_v['Valor']]).min(), pd.concat([df_i['Valor'], df_v['Valor']]).max()
    
    for m in meses:
        c1, c2 = dt.columns(2)
        with c1: render_cal(df_i[df_i['Data partida_dt'].dt.to_period('M')==m], 'Valor', f"IDA: {orig}->{dest}", 36.67, modo=="Pontos", g_min, g_max)
        with c2: render_cal(df_v[df_v['Data partida_dt'].dt.to_period('M')==m], 'Valor', f"VOLTA: {dest}->{orig}", 48.26, modo=="Pontos", g_min, g_max)
