...
def carregar_dados():
    try:
        ...
        # Tentativa robusta de leitura (tenta vírgula, se falhar ou não achar colunas, tenta tabulação)
        try:
            df = pd.read_csv(StringIO(conteudo_str))
            # Se a coluna 'ORIGEM' não estiver lá, o separador provavelmente é Tab (\t)
        ...
        # Limpeza robusta dos nomes das colunas
        df.columns = df.columns.str.strip().str.upper()
        
        # Verifica se as colunas essenciais existem antes do dropna
        colunas_essenciais = ['ORIGEM', 'DESTINO', 'DATA PARTIDA']
        for col in colunas_essenciais:
            if col not in df.columns:
                dt.error(f"Coluna essencial '{col}' não encontrada no arquivo CSV. Colunas lidas: {list(df.columns)}")
                return pd.DataFrame()
        ...
        # Cria as colunas agrupadas (SAO, RIO)
...
if df_voos.empty: 
    ...
    # Monta as rotas baseadas nas Localidades (ex: SAO -> NAT)
    ...
    # Filtro Dinâmico de Aeroportos para a IDA
    dt.sidebar.markdown("---")
    ...
    # Filtro Dinâmico de Aeroportos para a VOLTA
    ...
    # Aplica os filtros de Rota e Aeroporto selecionado
    ...
    # Lógica de renderização (Slades de Conexões e Tempo)
    dt.sidebar.markdown("---")
    dt.sidebar.subheader("✈️ Filtros de Tempo/Conexões")
    
    # SLIDERS IDA
    v_min_i, v_max_i = int(df_i_proc['NUMERO VOOS'].min() or 1), int(df_i_proc['NUMERO VOOS'].max() or 1)
    slide_v_i = dt.sidebar.slider("Ida: Conexões", v_min_i, v_max_i, (v_min_i, v_max_i)) if v_min_i != v_max_i else (v_min_i, v_max_i)
    t_min_i, t_max_i = round(df_i_proc['Duracao_Minutos'].min()/60, 1) if not df_i_proc.empty else 0.0, round(df_i_proc['Duracao_Minutos'].max()/60, 1) if not df_i_proc.empty else 0.0
    slide_t_i = dt.sidebar.slider("Ida: Tempo (h)", t_min_i, t_max_i, (t_min_i, t_max_i), step=0.5)
    
    # SLIDERS VOLTA
    v_min_v, v_max_v = int(df_v_proc['NUMERO VOOS'].min() or 1), int(df_v_proc['NUMERO VOOS'].max() or 1)
    slide_v_v = dt.sidebar.slider("Volta: Conexões", v_min_v, v_max_v, (v_min_v, v_max_v)) if v_min_v != v_max_v else (v_min_v, v_max_v)
    t_min_v, t_max_v = round(df_v_proc['Duracao_Minutos'].min()/60, 1) if not df_v_proc.empty else 0.0, round(df_v_proc['Duracao_Minutos'].max()/60, 1) if not df_v_proc.empty else 0.0
